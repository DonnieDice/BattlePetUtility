...
function addon:GetDatabrokerMenuData()
    return {
        ...
        },
        {
            ...
            func = function()
                self.db.global.Broker.ShowWoundedPets = not self.db.global.Broker.ShowWoundedPets
        ...
        },
        {
            ...
            func = function()
                self.db.global.Broker.ShowPetCharms = not self.db.global.Broker.ShowPetCharms
                addon:UpdateDatabrokerText()
            end,
            ...
            for slotIndex = 1, 3 do
                ...
                elseif not petID then
                    ...
                    tooltip:AddDoubleLine(
                        string.format("%s %s[%d] %s|r",
                            ICON_PATTERN_16:format(displayIcon),
                            rarityColor.hex or "|cffffffff",
                            tonumber(level) or 0,
                            displayName),
                    ...
                    local statsLine
                    if not isHurt then
                        statsLine = string.format("%s |cffffffff%d|r %s |cffffffff%d|r %s |cffffffff%d|r",
                            TEX_HEALTH_ICON, tonumber(maxHealth) or 0,
                            TEX_POWER_ICON,  tonumber(power)     or 0,
                            TEX_SPEED_ICON,  tonumber(speed)     or 0)
                    else
                        statsLine = string.format("%s |cffff5555%d|r %s |cffffffff%d|r %s |cffffffff%d|r",
                            TEX_HEALTH_ICON, tonumber(health) or 0,
                            TEX_POWER_ICON,  tonumber(power)  or 0,
                            TEX_SPEED_ICON,  tonumber(speed)  or 0)
                    end
            ...
            if addon.db.global.AutoSummonPet then
                if addon.db.char.AutoSummonLastPetID then
                    ...
                    if speciesID then
                        ...
                        local rarityColor = GetSafeRarityColor(rarity)
                        tooltip:AddDoubleLine("Current Auto Summon Companion",
                            string.format("%s[%d] %s|r", rarityColor.hex or "|cffffffff", tonumber(level) or 0, customName or speciesName or "Unknown Pet"))
                    else
                        addon.db.char.AutoSummonLastPetID = nil
...
function addon:UpdateDatabrokerText()
    ...
    if self.db.global.Broker.ShowWoundedPets then
        ...
        if woundedPets > 0 then
            table.insert(strings, string.format("%d |cffff5555wounded|r", woundedPets))
        end
    end

    if addon.db.global.AutoSummonPet and not self.db.char.AutoSummonLastPetID then
        table.insert(strings, "|cffff5555No companion|r")
    end
    ...
    if self.db.global.Broker.ShowPetCharms then
        ...
        table.insert(strings, string.format("%d %s", charmsNumAmount, petCharmIconString))
    end
